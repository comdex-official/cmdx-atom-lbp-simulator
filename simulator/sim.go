...
func Simulate(
	...
	// funderCoins = funderCoins.Add(sdk.NewCoin("uatom", sdk.NewInt(1_000_000_000))) // pool creation fee is 1,000
	...
	for currentTime.Before(endTime) {
		...
		spotPrice, err := app.GAMMKeeper.CalculateSpotPriceWithSwapFee(ctx, poolId, "uatom", "ucmdx")
		if err != nil {
			return nil, err
		}
...
func ExecuteBuys(ctx sdk.Context, keeper gammkeeper.Keeper, poolId uint64,
	...
	for i := 0; i < len(simulatedBuys); i++ {
		...
		msg, err := CreateSwapExactAmountIn(buyer, sdk.NewInt64Coin("uatom", currentBuy.Amount*1_000_000), pool)
		if err != nil {
			return simulatedBuys, totalAmount, buys, err
		}
